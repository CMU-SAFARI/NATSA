...
template<class DTYPE>
void SCRIMP(DTYPE* T, const int timeSeriesLength, const int windowSize, DTYPE* profile, unsigned int* profile_idxs,  mp_entry *profile_with_idx)
{
  ...
  cudaDeviceSynchronize();
  now = clock();
  printf("Time taken for matrix profile sample: %lf seconds\n", (now - start) / (double) CLOCKS_PER_SEC);
...
int main(int argc, char **argv)
{
  ...
  // Do SCRIMP
  SCRIMP<double>(T, T_host.size(), windowSize, profile, idxs, profile_with_idx);
